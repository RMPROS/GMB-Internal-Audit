...
function ScoreRing({ score, grade, size = 120 }) {
  ...
  const color = gradeColor[grade] || '#FF8C00';
  return (
    <svg width={size} height={size} style={{ flexShrink: 0 }}>
      ...
      <text x={size/2} y={size/2 - 6} textAnchor="middle" fill="#0A2342"
        style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize: size * 0.28, fontWeight: 700 }}>{score}</text>
      <text x={size/2} y={size/2 + 14} textAnchor="middle" fill={color}
        style={{ fontSize: size * 0.12, fontWeight: 800, letterSpacing: 1 }}>{grade}-GRADE</text>
    </svg>
  );
}

function SectionCard({ sec }) {
  const sc = sec.score >= 75 ? '#16a34a' : sec.score >= 50 ? '#FF8C00' : '#dc2626';
  return (
    <div style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 14, overflow: 'hidden', marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: '#f8f9fb', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 20 }}>{sec.icon}</span>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#0A2342' }}>{sec.title}</div>
            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 1 }}>{sec.subtitle}</div>
          </div>
        </div>
        <div style={{ fontSize: 13, fontWeight: 700, padding: '4px 14px', borderRadius: 100, background: sc + '18', color: sc }}>{sec.score}/100</div>
      </div>
      <div>
        {sec.findings.map((f, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '13px 18px', borderBottom: i < sec.findings.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#0A2342', marginBottom: 2 }}>{f.title}</div>
...
export default function InternalAuditTool() {
  ...
  const runAudit = async () => {
    ...
    try {
      ...
      const entry = { id: Date.now(), bizName: selectedBiz.name, address: selectedBiz.address, score: data.audit.overallScore, grade: data.audit.grade, audit: data.audit, runAt: new Date().toISOString() };
  ...
  const inputStyle = { border: '1.5px solid #e5e7eb', borderRadius: 10, padding: '11px 14px', fontFamily: 'inherit', fontSize: 14, color: '#0A2342', background: '#fff', outline: 'none', width: '100%', boxSizing: 'border-box', transition: 'border-color .2s' };
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tool-layout { display: grid; grid-template-columns: 260px 1fr; min-height: 100vh; }
        .sidebar { background: #1e293b; display: flex; flex-direction: column; }
        .main-area { padding: 32px 36px; overflow-y: auto; }
        ...
        .nav-btn.active { background: rgba(255,140,0,.15); color: #fff; border-right: 3px solid #FF8C00; }
        .card { background: #fff; border: 1.5px solid #e5e7eb; border-radius: 16px; padding: 24px; }
      ...
      <div className="tool-layout">
        ...
        <div className="sidebar">
          <div style={{ padding: '28px 20px 20px', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
            ...
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#FF8C00' }}>Internal GMB Tool</div>
          </div>
          <div style={{ padding: '12px 0', flex: 1 }}>
            ...
            <button className={`nav-btn${activeTab === 'history' ? ' active' : ''}`} onClick={() => setActiveTab('history')}>
              ...
              {history.length > 0 && <span style={{ marginLeft: 'auto', background: 'rgba(255,140,0,.3)', color: '#FF8C00', fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 100 }}>{history.length}</span>}
            </button>
          </div>
          {history.length > 0 && (
            <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,.07)' }}>
              ...
              {[{ label: 'Audits Run', value: history.length }, { label: 'Avg Score', value: Math.round(history.reduce((a, b) => a + b.score, 0) / history.length) }].map(s => (
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,.07)', fontSize: 10, color: 'rgba(255,255,255,.18)', lineHeight: 1.6 }}>Internal use only · v2.0</div>
        </div>
          ...
          {activeTab === 'audit' && (
            <div style={{ maxWidth: 860, margin: '0 auto', animation: 'rise .3s ease both' }}>
              <div style={{ marginBottom: 28 }}>
                <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 34, color: '#0A2342', letterSpacing: 0.5 }}>GMB Audit Tool</h1>
                <p style={{ fontSize: 13.5, color: '#6b7280', marginTop: 4 }}>Search any business, run an AI-powered audit, and save the report as a PDF.</p>
              </div>

              {/* Search card */}
              <div className="card" style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#0A2342', marginBottom: 12 }}>🔍 Business Search</div>
                <div ref={ddRef} style={{ position: 'relative', marginBottom: 12 }}>
                  ...
                  <input value={query} onChange={handleQueryChange} onFocus={() => results.length && setDdOpen(true)}
                    placeholder="Search for a business name…" style={{ ...inputStyle, paddingLeft: 42 }} />
                  {searching && <div style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', width: 16, height: 16, border: '2.5px solid #e5e7eb', borderTopColor: '#FF8C00', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />}
                  {ddOpen && results.length > 0 && (
                    <div style={{ position: 'absolute', left: 0, right: 0, top: 'calc(100% + 6px)', background: '#fff', border: '2px solid #e5e7eb', borderRadius: 12, boxShadow: '0 12px 36px rgba(0,0,0,.1)', zIndex: 200, overflow: 'hidden', maxHeight: 280, overflowY: 'auto' }}>
                      {results.map((r, i) => (
                        <div key={r.place_id || i} onMouseDown={() => selectBiz(r)}
                          ...
                          <div style={{ width: 30, height: 30, background: '#0A2342', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>🏢</div>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#0A2342' }}>{r.name}</div>
                            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 1 }}>{r.formatted_address}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {selectedBiz && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'linear-gradient(135deg,#0A2342,#0d3566)', borderRadius: 11, padding: '12px 16px', marginBottom: 14 }}>
                ...
                {error && <div style={{ background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: 9, padding: '10px 14px', fontSize: 13, color: '#dc2626', marginBottom: 12 }}>{error}</div>}

                <button onClick={runAudit} disabled={running || !selectedBiz}
                  style={{ width: '100%', padding: '13px 0', border: 'none', borderRadius: 11, fontFamily: 'inherit', fontSize: 14, fontWeight: 800, cursor: running || !selectedBiz ? 'not-allowed' : 'pointer', background: running || !selectedBiz ? '#e5e7eb' : '#FF8C00', color: running || !selectedBiz ? '#6b7280' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .2s', boxShadow: !running && selectedBiz ? '0 4px 16px rgba(255,140,0,.3)' : 'none' }}>
                  {running ? <><div style={{ width: 16, height: 16, border: '2.5px solid rgba(255,255,255,.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .7s linear infinite' }} /> Running Audit…</> : '▶  Run Audit'}
                </button>
              </div>

              {/* Progress */}
              {running && (
                <div className="card" style={{ marginBottom: 20, animation: 'rise .3s ease both' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: '#0A2342', animation: 'pulse 1.4s ease infinite' }}>{progMsg}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#FF8C00' }}>{progress}%</span>
                  </div>
                  <div style={{ height: 8, background: '#e5e7eb', borderRadius: 100, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg,#FF8C00,#ffb347)', borderRadius: 100, transition: 'width .6s ease' }} />
                  </div>
                </div>
              )}
              ...
              {/* Results */}
              {auditResult && !running && (
                <div ref={reportRef} style={{ animation: 'rise .4s ease both' }}>
                  {/* Score header */}
                  <div className="card" style={{ background: 'linear-gradient(135deg,#0A2342,#0d3566)', border: 'none', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
                    <ScoreRing score={auditResult.overallScore} grade={auditResult.grade} size={130} />
                    <div style={{ flex: 1, minWidth: 200 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {auditResult.sections.map(s => {
                          const c = s.score >= 75 ? '#16a34a' : s.score >= 50 ? '#FF8C00' : '#dc2626';
                          return (
                    ...
                    <button onClick={savePdf} disabled={savingPdf}
                      ...
                      {savingPdf ? <><div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .7s linear infinite' }} /> Generating…</> : <><span>📄</span> Save PDF</>}
                    </button>
                  </div>

                  {/* Top Priorities */}
                  <div className="card" style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#9ca3af', marginBottom: 14 }}>🎯 Top Priorities</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {auditResult.topPriorities.map((p, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', background: '#fffbf5', border: '1.5px solid rgba(255,140,0,.2)', borderRadius: 10 }}>
                          <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#FF8C00', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{i + 1}</div>
                          <span style={{ fontSize: 13, color: '#0A2342', fontWeight: 500 }}>{p}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Section cards */}
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#9ca3af', marginBottom: 12 }}>📋 Detailed Findings</div>
          ...
          {activeTab === 'history' && (
            <div style={{ maxWidth: 860, margin: '0 auto', animation: 'rise .3s ease both' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <div>
                  <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 34, color: '#0A2342', letterSpacing: 0.5 }}>Audit History</h1>
                  <p style={{ fontSize: 13.5, color: '#6b7280', marginTop: 4 }}>{history.length} audit{history.length !== 1 ? 's' : ''} saved locally in this browser.</p>
                </div>
                {history.length > 0 && <button onClick={clearHistory} style={{ background: 'none', border: '1.5px solid #e5e7eb', borderRadius: 9, padding: '8px 16px', fontFamily: 'inherit', fontSize: 12, fontWeight: 600, color: '#6b7280', cursor: 'pointer' }}>Clear All</button>}
              </div>
              {history.length === 0 ? (
                <div className="card" style={{ textAlign: 'center', padding: '60px 24px' }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#0A2342', marginBottom: 6 }}>No audits yet</div>
                  <div style={{ fontSize: 13, color: '#9ca3af' }}>Run an audit and it will appear here.</div>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {history.map(entry => {
                    const gc = gradeColor[entry.grade] || '#FF8C00';
                    return (
                      <div key={entry.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16, cursor: 'pointer', transition: 'border-color .15s' }}
                        ...
                        <div style={{ width: 52, height: 52, borderRadius: 12, background: gc + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ fontSize: 20, fontWeight: 800, color: gc }}>{entry.score}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: '#0A2342', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{entry.bizName}</div>
                          <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>{entry.address}</div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          ...
                          <div style={{ fontSize: 10, color: '#9ca3af' }}>{new Date(entry.runAt).toLocaleDateString()}</div>
                        </div>
